...
sub_nodes:
    - name: browser_app
      path: node_0_3_0_browser_app/CORE_0_3_0_browser_app.md
    - name: scheme_handler
      path: node_0_3_1_scheme_handler/CORE_0_3_1_scheme_handler.md
    - name: executor
      path: node_0_3_2_executor/CORE_0_3_2_executor.md
---
...
the owner, so this section is a ROLLUP of what the two sub-nodes below now
state, added so that a reader who stops here still has a correct
picture. the owner settles it; nothing here contradicts what stood before.)*
...
|---|---|---|
| `emit` — a python block puts HTML in its own place | request time, before Chromium sees the page | [scheme_handler](node_0_3_1_scheme_handler/CORE_0_3_1_scheme_handler.md) |
| `page_path` — a block knows the file it is in | request time | [scheme_handler](node_0_3_1_scheme_handler/CORE_0_3_1_scheme_handler.md) |
...
| the click wire form becomes an attribute, and the old inline rewrite is repaired | request time | [scheme_handler](node_0_3_1_scheme_handler/CORE_0_3_1_scheme_handler.md) |
...
- **Why two and not one.** A block runs before there is a DOM, so it
  ...
  values in motion, in [executor](node_0_3_2_executor/CORE_0_3_2_executor.md).
